...
import (
	...
	"github.com/bars43ru/bus2map/internal/egts/config"
	"github.com/bars43ru/bus2map/internal/egts/worker"
	"github.com/bars43ru/bus2map/internal/wialonips/service"
...
func main() {
	...
	handlerWialonIPSStream := service.NewReciver(tracker)

	var workers []Worker
	workers = append(workers, worker.NewSender(tracker, cfg.CoordinateAddr, cfg.Source))

	tpcServer, err := tcp.New(cfg.EGTS.Addr, handlerWialonIPSStream)
	if err != nil {
